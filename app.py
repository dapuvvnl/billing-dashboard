...
<style>
    ...
    div[data-testid="stDownloadButton"] > button .kpi-card {
        cursor: pointer;
        width: 100%;
    }
...
def render_mr_analysis():
    ...
    if df_rdf_idf_mr is not None and len(df_rdf_idf_mr) > 0:
        ...
        # ---- FIX: Create rdf_merge_df without duplicate NAME/MOBILE_NO ----
    ...
    # ---- KPI CARDS (NOW CLICKABLE DOWNLOAD BUTTONS) ----
    ...
    active_mr_df = mr_full[active_cols].rename(columns=active_rename)
    probe_zero_df = mr_full[mr_full['MOB_MRI'] == 0][active_cols].rename(columns=active_rename)
    auto_zero_df = mr_full[mr_full['AUTO'] == 0][active_cols].rename(columns=active_rename)

    # Same Location Data
    if df_same_location is not None and len(df_same_location) > 0:
        same_loc_download_df = df_same_location[df_same_location['TOTAL_BILLS'] > 0].copy()
        if df_agent_master is not None and len(df_agent_master) > 0:
            ...
            same_loc_download_df['NAME'] = ''
            same_loc_download_df['MOBILE_NO'] = ''
        
        # Merge with mr_summary for zone/circle/division
        if mr_summary is not None and len(mr_summary) > 0:
            same_loc_download_df = same_loc_download_df.merge(
            ...
            same_loc_download_df['ZONE_NAME'] = ''
            same_loc_download_df['CIRCLE_NAME'] = ''
            same_loc_download_df['DIV_NAME'] = ''
    ...
    with c1:
        st.download_button(
            label=f"""<div class="kpi-card blue"><div class="icon">👥</div><div class="value">{active_mr:,}</div><div class="label">Active Meter Readers</div></div>""",
            data=active_mr_df.to_csv(index=False).encode('utf-8'),
            file_name="active_meter_readers.csv",
            mime="text/csv",
            use_container_width=True,
            key="download_active_mr"
        )
    with c2:
        st.download_button(
            label=f"""<div class="kpi-card red"><div class="icon">📱❌</div><div class="value">{probe_zero:,}</div><div class="label">Probe Zero MR</div></div>""",
            data=probe_zero_df.to_csv(index=False).encode('utf-8'),
            file_name="probe_zero_mr.csv",
            mime="text/csv",
            use_container_width=True,
            key="download_probe_zero"
        )
    with c3:
        st.download_button(
            label=f"""<div class="kpi-card orange"><div class="icon">🤖❌</div><div class="value">{auto_zero:,}</div><div class="label">Auto Zero MR</div></div>""",
            data=auto_zero_df.to_csv(index=False).encode('utf-8'),
            file_name="auto_zero_mr.csv",
            mime="text/csv",
            use_container_width=True,
            key="download_auto_zero"
        )
    with c4:
        st.download_button(
            label=f"""<div class="kpi-card purple"><div class="icon">📍</div><div class="value">{same_location_mr_count:,}</div><div class="label">Same Location MR (10+)</div></div>""",
            data=same_loc_download_df.to_csv(index=False).encode('utf-8'),
            file_name="same_location_mr.csv",
            mime="text/csv",
            use_container_width=True,
            key="download_same_location"
        )
    st.markdown("---")

    # ======================== Average Bills per Day per Agent Distribution ========================
    ...
    # ---- 2. TOP 25 TOTAL RDF ----
    if show_top_rdf_mr and len(rdf_merge_df) > 0 and rdf_merge_df['RDF_COUNT'].sum() > 0:
    ...
    # ---- 3. TOP 25 TOTAL IDF ----
    if show_top_idf_mr and len(rdf_merge_df) > 0 and rdf_merge_df['IDF_COUNT'].sum() > 0:
    ...
    # ---- 4. TOP 25 NEWLY RDF ----
    if show_top_newly_rdf_mr and len(rdf_merge_df) > 0 and rdf_merge_df['NEWLY_RDF_COUNT'].sum() > 0:
    ...
    if show_probe_zero_top:
        cols_to_keep = ['AGENT_ID', 'NAME', 'MOBILE_NO', 'BILLED', 'MOB_MRI', 'OCR', 'AUTO', 
                        'ZONE_NAME', 'CIRCLE_NAME', 'DIV_NAME']
        top = mr_full[mr_full['MOB_MRI'] == 0][cols_to_keep].sort_values('BILLED', ascending=False).head(25).copy()
        top.rename(columns={
            'AGENT_ID': 'Agent ID', 'NAME': 'Name', 'MOBILE_NO': 'Mobile No',
            'BILLED': 'Total Bills', 'MOB_MRI': 'Probe', 'OCR': 'OCR', 'AUTO': 'Auto',
            'ZONE_NAME': 'Zone', 'CIRCLE_NAME': 'Circle', 'DIV_NAME': 'Division'
    ...
    if show_auto_zero_top:
        cols_to_keep = ['AGENT_ID', 'NAME', 'MOBILE_NO', 'BILLED', 'MOB_MRI', 'OCR', 'AUTO', 
                        'ZONE_NAME', 'CIRCLE_NAME', 'DIV_NAME']
        top = mr_full[mr_full['AUTO'] == 0][cols_to_keep].sort_values('BILLED', ascending=False).head(25).copy()
        top.rename(columns={
            'AGENT_ID': 'Agent ID', 'NAME': 'Name', 'MOBILE_NO': 'Mobile No',
            'BILLED': 'Total Bills', 'MOB_MRI': 'Probe', 'OCR': 'OCR', 'AUTO': 'Auto',
            'ZONE_NAME': 'Zone', 'CIRCLE_NAME': 'Circle', 'DIV_NAME': 'Division'
