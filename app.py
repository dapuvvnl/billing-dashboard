...
with st.spinner("📂 Loading data..."):
    ...
    df_agent_master = load_agent_master()  # New
...
def render_mr_analysis():
    ...
    if df_agent_master is not None and len(df_agent_master) > 0:
        # Merge with mr_summary to add NAME and MOBILE_NO
    ...
    if df_rdf_idf_mr is not None and len(df_rdf_idf_mr) > 0:
        ...
        # Merge agent master with rdf_idf_summary
        if df_agent_master is not None and len(df_agent_master) > 0:
        ...
        # Merge agent master again to ensure NAME/MOBILE in mr_full
        if df_agent_master is not None and len(df_agent_master) > 0:
            # If already have NAME/MOBILE from mr_summary, keep them
            # But to be safe, merge again (will not override if already present)
            mr_full = mr_full.merge(
                df_agent_master[['AGENT_ID', 'NAME', 'MOBILE_NO']],
                on='AGENT_ID',
                how='left',
                suffixes=('', '_master')
            )
            # Fill missing from master if empty
            if 'NAME_master' in mr_full.columns:
                mr_full['NAME'] = mr_full['NAME'].fillna(mr_full['NAME_master'])
                mr_full.drop(columns=['NAME_master'], inplace=True)
            if 'MOBILE_NO_master' in mr_full.columns:
                mr_full['MOBILE_NO'] = mr_full['MOBILE_NO'].fillna(mr_full['MOBILE_NO_master'])
                mr_full.drop(columns=['MOBILE_NO_master'], inplace=True)
            mr_full['NAME'] = mr_full['NAME'].fillna('')
            mr_full['MOBILE_NO'] = mr_full['MOBILE_NO'].fillna('')
    ...
    if df_same_location is not None and len(df_same_location) > 0:
        ...
        # Merge agent master for same_location_mr
        if df_agent_master is not None and len(df_agent_master) > 0:
    ...
    if search_agent:
        ...
        if len(billed_match) > 0 or len(rdf_match) > 0:
            ...
            if len(rdf_match) > 0:
                ...
                # Merge agent master to get NAME/MOBILE for rdf_result
                if df_agent_master is not None and len(df_agent_master) > 0:
            ...
            # Merge to create search_result
            search_result = pd.merge(billed_result, rdf_result, on='AGENT_ID', how='outer', suffixes=('', '_rdf'))
            # Combine NAME and MOBILE_NO from both sides if one side missing
    ...
    # Merge agent master to include NAME and MOBILE_NO
    if df_agent_master is not None and len(df_agent_master) > 0:
    ...
    if show_top_bill_mr:
        ...
        # Ensure NAME and MOBILE_NO are present
        if 'NAME' not in top.columns:
            top['NAME'] = ''
        if 'MOBILE_NO' not in top.columns:
            top['MOBILE_NO'] = ''
        top.columns = [
        ...
        # Reorder to put Name and Mobile No early
        cols_order = ['Agent ID', 'Name', 'Mobile No', 'Total Bills', 'Probe', 'OCR', 'Auto', 'Zone', 'Circle', 'Division']
        display_mr_table(top, "🏆 Top 25 MR by Total Bills", cols_order, "top25_total_bills")
    ...
    if show_top_rdf_mr:
        if len(rdf_merge_df) > 0 and rdf_merge_df['RDF_COUNT'].sum() > 0:
            ...
            display_data = top[['AGENT_ID', 'NAME', 'MOBILE_NO', 'RDF_COUNT', 'BILLED', 'MOB_MRI', 'OCR', 'AUTO', 'ZONE_NAME', 'CIRCLE_NAME', 'DIV_NAME']].copy()
            display_data.columns = ['Agent ID', 'Name', 'Mobile No', 'RDF Count', 'Total Bills', 'Probe', 'OCR', 'Auto', 'Zone', 'Circle', 'Division']
    ...
    if show_top_idf_mr:
        if len(rdf_merge_df) > 0 and rdf_merge_df['IDF_COUNT'].sum() > 0:
            ...
            display_data = top[['AGENT_ID', 'NAME', 'MOBILE_NO', 'IDF_COUNT', 'BILLED', 'MOB_MRI', 'OCR', 'AUTO', 'ZONE_NAME', 'CIRCLE_NAME', 'DIV_NAME']].copy()
            display_data.columns = ['Agent ID', 'Name', 'Mobile No', 'IDF Count', 'Total Bills', 'Probe', 'OCR', 'Auto', 'Zone', 'Circle', 'Division']
    ...
    if show_top_newly_rdf_mr:
        if len(rdf_merge_df) > 0 and rdf_merge_df['NEWLY_RDF_COUNT'].sum() > 0:
            ...
            display_data = top[['AGENT_ID', 'NAME', 'MOBILE_NO', 'NEWLY_RDF_COUNT', 'RDF_COUNT', 'BILLED', 'MOB_MRI', 'OCR', 'AUTO', 'ZONE_NAME', 'CIRCLE_NAME', 'DIV_NAME']].copy()
            display_data.columns = ['Agent ID', 'Name', 'Mobile No', 'Newly RDF', 'Total RDF', 'Total Bills', 'Probe', 'OCR', 'Auto', 'Zone', 'Circle', 'Division']
    ...
    if show_same_location_top:
        if len(same_location_mr) > 0:
            ...
            same_loc_top.columns = ['Div Code', 'Agent ID', 'Same Location Bills', 'Total Bills', 'Probe', 'OCR', 'Auto', 'Zone', 'Circle', 'Division', 'Name', 'Mobile No']
