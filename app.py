...
def render_mr_analysis():
    ...
    # ============================================================
    # MR AVERAGE DAILY BILL
    # Agent ID jitni baar repeat hota hai = working days.
    ...
    # ============================================================
    # ========== MR AVERAGE DAILY BILL CATEGORIES ==========
    # ============================================================
    st.markdown(
    ...
    category_counts = (
        mr_daily_avg['CATEGORY']
        .value_counts()
        .reindex(category_order, fill_value=0)
    )

    def show_avg_category(category):
        category_df = mr_daily_avg[
            mr_daily_avg['CATEGORY'] == category
        ].copy().sort_values(
            'AVG_DAILY_BILL', ascending=False
        )

        download_df = category_df[
            [
                'AGENT_ID', 'ZONE', 'CIRCLE', 'DIVISION',
                'TOTAL_BILLS', 'WORKING_DAYS',
                'AVG_DAILY_BILL', 'CATEGORY'
            ]
        ].copy()

        download_df.columns = [
            'Agent ID', 'Zone', 'Circle', 'Division',
            'Total Bills', 'Working Days',
            'Average Daily Bills', 'Category'
        ]
        ...
        st.download_button(
            ...
            mime='text/csv',
            use_container_width=True,
            key='download_' + category.replace(' ', '_')
        )

    # Five category boxes in one row.
    cat_cols = st.columns(5)

    for col, category in zip(cat_cols, category_order):
        ...
        with col:
            if st.button(
                f"📊 {category}\n{count:,} MR ({pct:.1f}%)",
                key='avg_category_' + category.replace(' ', '_'),
                use_container_width=True
            ):
                st.session_state['selected_avg_category'] = category

    selected_avg_category = st.session_state.get(
        'selected_avg_category'
    )

    if selected_avg_category:
        count = int(category_counts[selected_avg_category])
        pct = (count / active_mr * 100) if active_mr else 0

        st.markdown(
            f"### 📋 {selected_avg_category} — "
            f"{count:,} MR ({pct:.1f}% of Active MR)"
        )

        show_avg_category(selected_avg_category)
