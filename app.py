...
def render_mr_analysis():
    ...
    if df_rdf_idf_mr is not None and len(df_rdf_idf_mr) > 0:
        ...
        # rdf_idf_summary already has NAME and MOBILE_NO, so we exclude them from mr_summary selection
        rdf_merge_df = pd.merge(
            rdf_idf_summary,
            mr_summary[['AGENT_ID', 'BILLED', 'MOB_MRI', 'OCR', 'AUTO', 'ZONE_NAME', 'CIRCLE_NAME', 'DIV_NAME']],  # remove NAME, MOBILE_NO
            on='AGENT_ID',
            how='inner'
        )
        # Now rdf_merge_df has NAME and MOBILE_NO from rdf_idf_summary only
    else:
