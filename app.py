...
<style>
    ...
    /* Style for download buttons below cards */
    .download-btn-small {
...
if df_billed is None or df_billable is None:
    ...
    - `agent_master.xlsx` or `agent_master.csv` (optional)
    - `requirements.txt`
    """)
    st.stop()
...
def render_mr_analysis():
    ...
    if df_rdf_idf_mr is not None and len(df_rdf_idf_mr) > 0:
        ...
        # ---- Create rdf_merge_df without duplicate NAME/MOBILE_NO ----
    ...
    # ---- SAME LOCATION (FIXED: include LATITUDE and LONGITUDE) ----
    ...
    with st.container():
        ...
        # Determine column names (same logic as home)
        zone_col = 'ZONE' if 'ZONE' in mr_full.columns else 'ZONE_NAME' if 'ZONE_NAME' in mr_full.columns else None
        circle_col = 'CIRCLE' if 'CIRCLE' in mr_full.columns else 'CIRCLE_NAME' if 'CIRCLE_NAME' in mr_full.columns else None
        div_col = 'DIVISION' if 'DIVISION' in mr_full.columns else 'DIV_NAME' if 'DIV_NAME' in mr_full.columns else None

        # Zone filter
        if zone_col:
            with col1:
                zone_options = ['Overall'] + sorted(mr_full[zone_col].dropna().unique().tolist())
                selected_zone = st.selectbox("🏢 Zone", zone_options, key="zone_filter_mr")
            zone_df = mr_full.copy() if selected_zone == 'Overall' else mr_full[mr_full[zone_col] == selected_zone]
        else:
            zone_df = mr_full.copy()
            selected_zone = 'Overall'

        # Circle filter
        if circle_col:
            with col2:
                circle_options = ['All Circles'] + sorted(zone_df[circle_col].dropna().unique().tolist())
                selected_circle = st.selectbox("🔵 Circle", circle_options, key="circle_filter_mr")
            circle_df = zone_df.copy() if selected_circle == 'All Circles' else zone_df[zone_df[circle_col] == selected_circle]
        else:
            circle_df = zone_df.copy()
            selected_circle = 'All Circles'

        # Division filter
        if div_col:
            with col3:
                div_options = ['All Divisions'] + sorted(circle_df[div_col].dropna().unique().tolist())
                selected_division = st.selectbox("📂 Division", div_options, key="div_filter_mr")
            filtered_agents_df = circle_df.copy() if selected_division == 'All Divisions' else circle_df[circle_df[div_col] == selected_division]
        else:
            filtered_agents_df = circle_df.copy()
            selected_division = 'All Divisions'

        st.markdown('</div>', unsafe_allow_html=True)

    # Get filtered agent IDs
    filtered_agent_ids = filtered_agents_df['AGENT_ID'].unique()

    # ---- MR SEARCH (independent of filters) ----
    ...
    if search_agent:
        ...
        # Search in mr_full (unfiltered) so we can find any agent
        billed_match = mr_full[mr_full['AGENT_ID'].eq(search_key)].copy()
        if len(billed_match) == 0:
            billed_match = mr_full[mr_full['AGENT_ID'].str.contains(search_key, case=False, na=False, regex=False)].copy()
        if len(billed_match) > 0:
            # Show result
            result_cols = ['AGENT_ID', 'NAME', 'MOBILE_NO', 'ZONE_NAME', 'CIRCLE_NAME', 'DIV_NAME', 'BILLED', 'MOB_MRI', 'OCR', 'IDF_COUNT', 'RDF_COUNT', 'NEWLY_RDF_COUNT']
            result_df = billed_match[result_cols].copy()
            result_df.rename(columns={
                'AGENT_ID': 'Agent ID', 'NAME': 'Name', 'MOBILE_NO': 'Mobile No',
                'ZONE_NAME': 'Zone', 'CIRCLE_NAME': 'Circle', 'DIV_NAME': 'Division',
                'BILLED': 'Total Bill', 'MOB_MRI': 'Probe', 'OCR': 'OCR',
                'IDF_COUNT': 'IDF Count', 'RDF_COUNT': 'RDF Count', 'NEWLY_RDF_COUNT': 'Newly RDF'
            }, inplace=True)
    ...
    # ---- Compute metrics based on filtered agents ----
    # Filter mr_full, rdf_merge_df, same_location_mr, agent_stats
    mr_full_filtered = mr_full[mr_full['AGENT_ID'].isin(filtered_agent_ids)]
    rdf_merge_filtered = rdf_merge_df[rdf_merge_df['AGENT_ID'].isin(filtered_agent_ids)] if len(rdf_merge_df) > 0 else pd.DataFrame()
    same_location_filtered = same_location_mr[same_location_mr['AGENT_ID'].isin(filtered_agent_ids)] if len(same_location_mr) > 0 else pd.DataFrame()

    active_mr = len(mr_full_filtered)
    probe_zero = int((mr_full_filtered['MOB_MRI'] == 0).sum())
    auto_zero = int((mr_full_filtered['AUTO'] == 0).sum())
    same_location_mr_count = same_location_filtered['AGENT_ID'].nunique() if len(same_location_filtered) > 0 else 0

    # ---- KPI CARDS (with download buttons) ----
    # Prepare data for downloads (filtered)
    active_cols = ['AGENT_ID', 'NAME', 'MOBILE_NO', 'ZONE_NAME', 'CIRCLE_NAME', 'DIV_NAME', 'BILLED', 'MOB_MRI', 'OCR']
    active_rename = {
        'AGENT_ID': 'Agent ID', 'NAME': 'Name', 'MOBILE_NO': 'Mobile No',
        'ZONE_NAME': 'Zone', 'CIRCLE_NAME': 'Circle', 'DIV_NAME': 'Division',
        'BILLED': 'Total Bill', 'MOB_MRI': 'Probe', 'OCR': 'OCR'
    ...
    active_mr_df = mr_full_filtered[active_cols].rename(columns=active_rename)
    probe_zero_df = mr_full_filtered[mr_full_filtered['MOB_MRI'] == 0][active_cols].rename(columns=active_rename)
    auto_zero_df = mr_full_filtered[mr_full_filtered['AUTO'] == 0][active_cols].rename(columns=active_rename)

    # Same Location Data (filtered)
    if len(same_location_filtered) > 0:
        # Merge with mr_summary for zone/circle/division if not already present
        if 'ZONE_NAME' not in same_location_filtered.columns:
            same_loc_download_df = same_location_filtered.merge(
                mr_summary[['AGENT_ID', 'ZONE_NAME', 'CIRCLE_NAME', 'DIV_NAME']],
                on='AGENT_ID',
                how='left'
            )
        else:
            same_loc_download_df = same_location_filtered.copy()
        # Select columns (LATITUDE and LONGITUDE are already present)
        same_loc_download_df = same_loc_download_df[[
            'LATITUDE', 'LONGITUDE', 'AGENT_ID', 'NAME', 'MOBILE_NO',
            'DIV_CODE', 'ZONE_NAME', 'CIRCLE_NAME', 'DIV_NAME', 'TOTAL_BILLS'
        ]].rename(columns={
            'LATITUDE': 'Latitude', 'LONGITUDE': 'Longitude',
            'AGENT_ID': 'Agent ID', 'NAME': 'Name', 'MOBILE_NO': 'Mobile No',
            'DIV_CODE': 'Div Code', 'ZONE_NAME': 'Zone',
            'CIRCLE_NAME': 'Circle', 'DIV_NAME': 'Division',
            'TOTAL_BILLS': 'Total Bills'
        })
    else:
        same_loc_download_df = pd.DataFrame(columns=[
            'Latitude', 'Longitude', 'Agent ID', 'Name', 'Mobile No',
            'Div Code', 'Zone', 'Circle', 'Division', 'Total Bills'
        ])

    # Display KPI cards with download buttons below
    c1, c2, c3, c4 = st.columns(4)
    ...
    with c1:
        ...
        st.download_button(
            ...
            data=active_mr_df.to_csv(index=False).encode('utf-8'),
            file_name="active_meter_readers.csv",
            mime="text/csv",
            use_container_width=True,
            key="download_active_mr_v2"
        )
    ...
    with c2:
        ...
        st.download_button(
            label="📥 Download Raw Data",
            data=probe_zero_df.to_csv(index=False).encode('utf-8'),
            file_name="probe_zero_mr.csv",
            mime="text/csv",
            use_container_width=True,
            key="download_probe_zero_v2"
        )
    ...
    with c3:
        ...
        st.download_button(
            label="📥 Download Raw Data",
            data=auto_zero_df.to_csv(index=False).encode('utf-8'),
            file_name="auto_zero_mr.csv",
            mime="text/csv",
            use_container_width=True,
            key="download_auto_zero_v2"
        )
    ...
    with c4:
        ...
        st.download_button(
            label="📥 Download Raw Data",
            data=same_loc_download_df.to_csv(index=False).encode('utf-8'),
            file_name="same_location_mr.csv",
            mime="text/csv",
            use_container_width=True,
            key="download_same_location_v2"
        )

    st.markdown("---")

    # ---- Average Bills per Day per Agent Distribution (filtered) ----
    st.markdown('<div class="section-title">📊 <span>Average Bills per Day per Agent Distribution</span></div>', unsafe_allow_html=True)

    # Compute agent_stats from df_excel (raw data) but filter by filtered_agent_ids
    agent_stats = df_excel.groupby('AGENT_ID').agg(
        total_bills=('BILLED', 'sum'),
        days=('BILLED', 'count')
    ).reset_index()
    # Filter by filtered_agent_ids
    agent_stats = agent_stats[agent_stats['AGENT_ID'].isin(filtered_agent_ids)]
    ...
    total_active = len(mr_full_filtered)
    ...
    # ---- TOP 25 BUTTONS (using filtered data) ----
    ...
    if show_top_bill_mr:
        ...
        top = mr_full_filtered[cols_to_keep].sort_values('BILLED', ascending=False).head(25).copy()
        top.rename(columns={
            'AGENT_ID': 'Agent ID', 'NAME': 'Name', 'MOBILE_NO': 'Mobile No',
            'BILLED': 'Total Bills', 'MOB_MRI': 'Probe', 'OCR': 'OCR', 'AUTO': 'Auto',
            'ZONE_NAME': 'Zone', 'CIRCLE_NAME': 'Circle', 'DIV_NAME': 'Division'
        }, inplace=True)
    ...
    # ---- 2. TOP 25 TOTAL RDF ----
    if show_top_rdf_mr and len(rdf_merge_filtered) > 0 and rdf_merge_filtered['RDF_COUNT'].sum() > 0:
        cols_to_keep = ['AGENT_ID', 'NAME', 'MOBILE_NO', 'RDF_COUNT', 'BILLED', 'MOB_MRI', 'OCR', 'AUTO', 
                        'ZONE_NAME', 'CIRCLE_NAME', 'DIV_NAME']
        top = rdf_merge_filtered[cols_to_keep].sort_values('RDF_COUNT', ascending=False).head(25).copy()
        top.rename(columns={
            'AGENT_ID': 'Agent ID', 'NAME': 'Name', 'MOBILE_NO': 'Mobile No',
            'RDF_COUNT': 'RDF Count', 'BILLED': 'Total Bills',
            'MOB_MRI': 'Probe', 'OCR': 'OCR', 'AUTO': 'Auto',
            'ZONE_NAME': 'Zone', 'CIRCLE_NAME': 'Circle', 'DIV_NAME': 'Division'
        }, inplace=True)
    ...
    # ---- 3. TOP 25 TOTAL IDF ----
    if show_top_idf_mr and len(rdf_merge_filtered) > 0 and rdf_merge_filtered['IDF_COUNT'].sum() > 0:
        cols_to_keep = ['AGENT_ID', 'NAME', 'MOBILE_NO', 'IDF_COUNT', 'BILLED', 'MOB_MRI', 'OCR', 'AUTO', 
                        'ZONE_NAME', 'CIRCLE_NAME', 'DIV_NAME']
        top = rdf_merge_filtered[cols_to_keep].sort_values('IDF_COUNT', ascending=False).head(25).copy()
        top.rename(columns={
            'AGENT_ID': 'Agent ID', 'NAME': 'Name', 'MOBILE_NO': 'Mobile No',
            'IDF_COUNT': 'IDF Count', 'BILLED': 'Total Bills',
            'MOB_MRI': 'Probe', 'OCR': 'OCR', 'AUTO': 'Auto',
            'ZONE_NAME': 'Zone', 'CIRCLE_NAME': 'Circle', 'DIV_NAME': 'Division'
        }, inplace=True)
    ...
    # ---- 4. TOP 25 NEWLY RDF ----
    if show_top_newly_rdf_mr and len(rdf_merge_filtered) > 0 and rdf_merge_filtered['NEWLY_RDF_COUNT'].sum() > 0:
        cols_to_keep = ['AGENT_ID', 'NAME', 'MOBILE_NO', 'NEWLY_RDF_COUNT', 'RDF_COUNT', 'BILLED', 'MOB_MRI', 'OCR', 'AUTO', 
                        'ZONE_NAME', 'CIRCLE_NAME', 'DIV_NAME']
        top = rdf_merge_filtered[cols_to_keep].sort_values('NEWLY_RDF_COUNT', ascending=False).head(25).copy()
        top.rename(columns={
            'AGENT_ID': 'Agent ID', 'NAME': 'Name', 'MOBILE_NO': 'Mobile No',
            'NEWLY_RDF_COUNT': 'Newly RDF', 'RDF_COUNT': 'Total RDF',
            'BILLED': 'Total Bills', 'MOB_MRI': 'Probe', 'OCR': 'OCR', 'AUTO': 'Auto',
            'ZONE_NAME': 'Zone', 'CIRCLE_NAME': 'Circle', 'DIV_NAME': 'Division'
        }, inplace=True)
    ...
    if show_probe_zero_top:
        cols_to_keep = ['AGENT_ID', 'NAME', 'MOBILE_NO', 'BILLED', 'MOB_MRI', 'OCR', 'AUTO', 
                        'ZONE_NAME', 'CIRCLE_NAME', 'DIV_NAME']
        top = mr_full_filtered[mr_full_filtered['MOB_MRI'] == 0][cols_to_keep].sort_values('BILLED', ascending=False).head(25).copy()
        top.rename(columns={
            'AGENT_ID': 'Agent ID', 'NAME': 'Name', 'MOBILE_NO': 'Mobile No',
            'BILLED': 'Total Bills', 'MOB_MRI': 'Probe', 'OCR': 'OCR', 'AUTO': 'Auto',
            'ZONE_NAME': 'Zone', 'CIRCLE_NAME': 'Circle', 'DIV_NAME': 'Division'
    ...
    if show_auto_zero_top:
        cols_to_keep = ['AGENT_ID', 'NAME', 'MOBILE_NO', 'BILLED', 'MOB_MRI', 'OCR', 'AUTO', 
                        'ZONE_NAME', 'CIRCLE_NAME', 'DIV_NAME']
        top = mr_full_filtered[mr_full_filtered['AUTO'] == 0][cols_to_keep].sort_values('BILLED', ascending=False).head(25).copy()
        top.rename(columns={
            'AGENT_ID': 'Agent ID', 'NAME': 'Name', 'MOBILE_NO': 'Mobile No',
            'BILLED': 'Total Bills', 'MOB_MRI': 'Probe', 'OCR': 'OCR', 'AUTO': 'Auto',
            'ZONE_NAME': 'Zone', 'CIRCLE_NAME': 'Circle', 'DIV_NAME': 'Division'
    ...
    if show_same_location_top:
        if len(same_location_filtered) > 0:
            # Remove NAME and MOBILE_NO from agent_details to avoid suffix conflict
            agent_details = mr_summary[['AGENT_ID', 'BILLED', 'MOB_MRI', 'OCR', 'AUTO', 'ZONE_NAME', 'CIRCLE_NAME', 'DIV_NAME']].copy()
            same_loc_top = pd.merge(same_location_filtered, agent_details, on='AGENT_ID', how='left')
            ...
            cols_to_keep = ['AGENT_ID', 'NAME', 'MOBILE_NO', 'DIV_CODE', 'TOTAL_BILLS', 'BILLED', 'MOB_MRI', 'OCR', 'AUTO', 
                            'ZONE_NAME', 'CIRCLE_NAME', 'DIV_NAME', 'LATITUDE', 'LONGITUDE']
            top = same_loc_top[cols_to_keep].copy()
            top.rename(columns={
                'AGENT_ID': 'Agent ID', 'NAME': 'Name', 'MOBILE_NO': 'Mobile No',
                'DIV_CODE': 'Div Code', 'TOTAL_BILLS': 'Same Location Bills',
                'BILLED': 'Total Bills', 'MOB_MRI': 'Probe', 'OCR': 'OCR', 'AUTO': 'Auto',
                'ZONE_NAME': 'Zone', 'CIRCLE_NAME': 'Circle', 'DIV_NAME': 'Division',
                'LATITUDE': 'Latitude', 'LONGITUDE': 'Longitude'
            }, inplace=True)
            display_mr_table(top, "📍 Top 25 Same Location MR",
                            ['Agent ID', 'Name', 'Mobile No', 'Div Code', 'Same Location Bills', 'Total Bills', 'Probe', 'OCR', 'Auto', 'Zone', 'Circle', 'Division', 'Latitude', 'Longitude'],
                            "top25_same_location")
        else:
