...
def render_mr_analysis():
    ...
    if search_agent:
        ...
        if len(billed_match) > 0 or len(rdf_match) > 0:
            if len(billed_match) > 0:
                billed_result = billed_match[['AGENT_ID', 'ZONE_NAME', 'CIRCLE_NAME', 'DIV_NAME', 'BILLED', 'MOB_MRI', 'OCR']].copy()
            else:
                billed_result = pd.DataFrame(columns=['AGENT_ID', 'ZONE_NAME', 'CIRCLE_NAME', 'DIV_NAME', 'BILLED', 'MOB_MRI', 'OCR'])
            ...
            if len(rdf_match) > 0:
                ...
                rdf_result = pd.DataFrame(columns=['AGENT_ID', 'RDF_COUNT', 'IDF_COUNT', 'NEWLY_RDF_COUNT'])

            search_result = pd.merge(billed_result, rdf_result, on='AGENT_ID', how='outer')
            ...
            search_result = search_result[['AGENT_ID', 'ZONE_NAME', 'CIRCLE_NAME', 'DIV_NAME', 'BILLED', 'MOB_MRI', 'OCR', 'IDF_COUNT', 'RDF_COUNT', 'NEWLY_RDF_COUNT']].copy()
            search_result.columns = ['Agent ID', 'Zone', 'Circle', 'Division', 'Total Bill', 'Probe Count', 'OCR Count', 'IDF Count', 'RDF Count', 'Newly RDF Count']
    ...
    # Check if BILL_DATE exists
    if 'BILL_DATE' in df_excel.columns:
        # Convert to date (ignore time)
        df_excel['BILL_DATE'] = pd.to_datetime(df_excel['BILL_DATE'], errors='coerce')
        # Group by agent and date to sum BILLED per day per agent
        agent_daily = df_excel.groupby(['AGENT_ID', df_excel['BILL_DATE'].dt.date]).agg({'BILLED': 'sum'}).reset_index()
        # Now compute total bills and distinct days per agent
        agent_stats = agent_daily.groupby('AGENT_ID').agg(
            total_bills=('BILLED', 'sum'),
            days=('BILL_DATE', 'nunique')  # number of distinct working days
        ).reset_index()
        # Keep only agents present in mr_summary (active agents)
        ...
        labels = ['1-10', '11-30', '31-50', '51-75', '75+']
        agent_stats['category'] = pd.cut(agent_stats['avg_per_day'], bins=bins, labels=labels, right=True)
        # Count per category (ensure all labels are present)
        ...
        st.info("ℹ️ BILL_DATE column not found in billed data. Cannot compute average bills per day.")
    # ===========================================================================================================
    ...
    if show_top_bill_mr:
        ...
        top.columns = [
            'Agent ID', 'Total Bills', 'Probe', 'OCR',
            'Zone', 'Circle', 'Division', 'Auto',
            'RDF Count', 'IDF Count', 'Newly RDF', 'Newly IDF',
            'Div Code', 'RDF/IDF Division', 'Total RDF', 'Total RDF/IDF'
        ]
    ...
    if show_top_rdf_mr:
        if len(rdf_merge_df) > 0 and rdf_merge_df['RDF_COUNT'].sum() > 0:
            ...
            display_data = top[['AGENT_ID', 'RDF_COUNT', 'BILLED', 'MOB_MRI', 'OCR', 'AUTO', 'ZONE_NAME', 'CIRCLE_NAME', 'DIV_NAME']].copy()
            display_data.columns = ['Agent ID', 'RDF Count', 'Total Bills', 'Probe', 'OCR', 'Auto', 'Zone', 'Circle', 'Division']
            display_mr_table(display_data, "🔴 Top 25 MR by Total RDF",
                            ['Agent ID', 'RDF Count', 'Total Bills', 'Probe', 'OCR', 'Auto', 'Zone', 'Circle', 'Division'])
        else:
    ...
    if show_top_idf_mr:
        if len(rdf_merge_df) > 0 and rdf_merge_df['IDF_COUNT'].sum() > 0:
            ...
            display_data = top[['AGENT_ID', 'IDF_COUNT', 'BILLED', 'MOB_MRI', 'OCR', 'AUTO', 'ZONE_NAME', 'CIRCLE_NAME', 'DIV_NAME']].copy()
            display_data.columns = ['Agent ID', 'IDF Count', 'Total Bills', 'Probe', 'OCR', 'Auto', 'Zone', 'Circle', 'Division']
            display_mr_table(display_data, "🟣 Top 25 MR by Total IDF",
                            ['Agent ID', 'IDF Count', 'Total Bills', 'Probe', 'OCR', 'Auto', 'Zone', 'Circle', 'Division'])
        else:
    ...
    if show_top_newly_rdf_mr:
        if len(rdf_merge_df) > 0 and rdf_merge_df['NEWLY_RDF_COUNT'].sum() > 0:
            ...
            display_data = top[['AGENT_ID', 'NEWLY_RDF_COUNT', 'RDF_COUNT', 'BILLED', 'MOB_MRI', 'OCR', 'AUTO', 'ZONE_NAME', 'CIRCLE_NAME', 'DIV_NAME']].copy()
            display_data.columns = ['Agent ID', 'Newly RDF', 'Total RDF', 'Total Bills', 'Probe', 'OCR', 'Auto', 'Zone', 'Circle', 'Division']
            display_mr_table(display_data, "🆕 Top 25 MR by Newly RDF",
                            ['Agent ID', 'Newly RDF', 'Total RDF', 'Total Bills', 'Probe', 'OCR', 'Auto', 'Zone', 'Circle', 'Division'])
        else:
    ...
    if show_same_location_top:
        if len(same_location_mr) > 0:
            ...
            same_loc_top = pd.merge(same_loc_top, agent_details, on='AGENT_ID', how='left')
            same_loc_top = same_loc_top.sort_values(['TOTAL_BILLS', 'AGENT_ID'], ascending=[False, True]).head(25)
            same_loc_top.columns = ['Div Code', 'Agent ID', 'Same Location Bills', 'Total Bills', 'Probe', 'OCR', 'Auto', 'Zone', 'Circle', 'Division']
            display_mr_table(same_loc_top, "📍 Top 25 Same Location MR",
                            ['Div Code', 'Agent ID', 'Same Location Bills', 'Total Bills', 'Probe', 'OCR', 'Auto', 'Zone', 'Circle', 'Division'])
        else:
